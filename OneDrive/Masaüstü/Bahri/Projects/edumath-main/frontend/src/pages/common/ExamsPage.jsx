              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {q.options.map((opt, i) => (
                  <label key={i} className={`flex items-center gap-3 p-4 border rounded-xl cursor-pointer transition-all ${userAnswers[q._id] === opt ? 'bg-indigo-50 border-indigo-500 ring-1 ring-indigo-500' : 'hover:bg-slate-50 dark:hover:bg-slate-700 border-slate-200 dark:border-slate-700'}`}>
                    <input type="radio" name={`q-${q._id}`} value={typeof opt === 'object' ? opt.text : opt} onChange={() => setUserAnswers({...userAnswers, [q._id]: typeof opt === 'object' ? opt.text : opt})} className="w-5 h-5 text-indigo-600"/>
                    <span className="text-slate-700 dark:text-slate-200">{typeof opt === 'object' ? opt.text : opt}</span>
                  </label>
                ))}
              </div>
            </div>
                  ...
                  <div key={idx} className={`bg-white dark:bg-slate-800 p-6 rounded-2xl border-2 transition-all ${isAnswered ? (state.isCorrect ? 'border-green-200' : 'border-red-200') : 'border-transparent shadow-sm'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                      ...
                      {(Array.isArray(q.options) ? q.options : Object.values(q.options)).map((opt, i) => {
                        ...
                        const isSelected = state?.selected === (q.options.A ? optKey : (typeof opt === 'object' ? opt.text : opt)); // Backend formatına göre kontrol
                        
                        let btnClass = "border-slate-200 hover:bg-slate-50"; // Default
                        if (isAnswered) {
                          if (q.correctAnswer === (q.options.A ? optKey : (typeof opt === 'object' ? opt.text : opt))) btnClass = "bg-green-100 border-green-500 text-green-800"; // Doğru şık
                        ...
                        return (
                          <button 
                            key={i} 
                            disabled={isAnswered}
                            onClick={() => handlePracticeAnswer(idx, q.options.A ? optKey : (typeof opt === 'object' ? opt.text : opt), q.correctAnswer)}
                            className={`p-4 border rounded-xl text-left transition-all ${btnClass}`}
                          >
                            <span className="font-bold mr-2">{optKey})</span> {typeof opt === 'object' ? opt.text : opt}
                          </button>
                        )
                      })}
                    </div>
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {exams.length === 0 ? <div className="col-span-full text-center py-10 text-slate-500">Henüz sınav oluşturulmamış.</div> : exams.map(exam => {
          // Öğrenci sınavı çözmüş mü kontrolü (örnek: results içinde studentId eşleşmesi)
          let hasResult = false;
          let myResult = null;
          if (role === 'student' && exam.results && Array.isArray(exam.results)) {
            const userId = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user'))._id : null;
            myResult = exam.results.find(r => r.studentId === userId);
            hasResult = !!myResult;
          }
          return (
            <div key={exam._id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-all group">
              ...
              {role === 'student' ? (
                hasResult ? (
                  <button
                    onClick={() => window.location.href = `/student/exams/${exam._id}/analysis`}
                    className="w-full bg-emerald-600 text-white py-2.5 rounded-xl font-medium hover:bg-emerald-700 flex items-center justify-center gap-2"
                  >
                    <CheckCircle size={18} /> Sonucu Gör
                  </button>
                ) : (
                  <button
                    onClick={() => startExam(exam._id)}
                    className="w-full bg-indigo-600 text-white py-2.5 rounded-xl font-medium hover:bg-indigo-700 flex items-center justify-center gap-2"
                  >
                    <Play size={18} /> Başla
                  </button>
                )
              ) : (
                <div className="w-full bg-slate-100 dark:bg-slate-700 text-slate-500 py-2.5 rounded-xl text-center text-sm font-medium">{exam.results.length} Katılım</div>
              )}
            </div>
          );
        })}
