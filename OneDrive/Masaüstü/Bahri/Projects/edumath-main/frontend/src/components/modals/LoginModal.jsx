...
const LoginModal = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (rawMsg.includes("Cannot set property") || rawMsg.includes("IncomingMessage")) {
          setError("Sunucu tarafında geçici bir sorun oluştu. Lütfen daha sonra tekrar deneyin.");
      } else {
          setError(rawMsg || "Bir hata oluştu. Bilgilerinizi kontrol edin.");
      }
    } finally {
      setLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {isLoginView && (
              <div className="flex justify-between items-center text-sm">
                ...
                <button type="button" className="text-indigo-600 font-medium hover:underline">Şifremi Unuttum?</button>
              </div>
            )}
