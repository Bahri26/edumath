...
const LineChartSimple = ({ data = [] }) => {
	...
	const onMove = (e) => {
		...
		for (let p of points) if (Math.abs(p.x - mx) < Math.abs(nearest.x - mx)) nearest = p;
		setTip({ x: nearest.x + 8, y: nearest.y - 30, label: nearest.d?.label || nearest.d?.name || 'P', value: nearest.d?.score });
	};
	const leave = () => setTip(null);

	return (
		<div className="relative" onMouseMove={onMove} onMouseLeave={leave}>
						...
						<g key={i}>
							<rect
								...
								onMouseMove={(e) => {
									...
									setTip({ x: e.clientX - rect.left + 8, y: e.clientY - rect.top - 24, label: d.label || d.name || `#${i+1}`, value: d.score });
								}}
