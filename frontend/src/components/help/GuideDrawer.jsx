...
export default function GuideDrawer({ audience = 'student', open, onClose }) {
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-slate-900/50 backdrop-blur-[2px] transition-opacity ${open ? 'opacity-100' : 'opacity-0'}`}
        onClick={onClose}
        ...
        <div className="relative overflow-hidden border-b border-slate-200 dark:border-slate-700">
          ...
          <div className="relative flex items-start justify-between gap-3 px-5 py-5 sm:px-6">
            ...
            <button
              type="button"
              onClick={onClose}
              className="shrink-0 rounded-xl border border-slate-200/80 bg-white/90 p-2 text-slate-600 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
