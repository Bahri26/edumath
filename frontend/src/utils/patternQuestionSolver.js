/**
 * Soru formunda otomatik doğru şık ve adım adım çözüm (backend ile aynı kurallar).
 */
...
export function solvePatternQuestion(input = {}) {
  const combined = [
    input.introText,
    input.questionText,
    input.text,
...
export function optionMatchesCorrect(opt, correctAnswer) {
  if (!correctAnswer || !String(opt || '').trim()) return false;
  if (opt === correctAnswer) return true;
  return normalizeOptionValue(opt) === normalizeOptionValue(correctAnswer);
}
