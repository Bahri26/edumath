...
async function findAll({ page = 1, limit = 20, q, filters = {}, sort } = {}) {
  ...
  const pk = cols.includes('id') ? 'id' : (cols.includes('student_answer_id') ? 'student_answer_id' : cols[0]);

  if (q) {
    if (cols.includes('answer_text')) qb.where('answer_text', 'like', '%' + q + '%');
...
async function findById(id) {
  const colInfo = await knex(TABLE).columnInfo();
  const pk = colInfo.id ? 'id' : (colInfo.student_answer_id ? 'student_answer_id' : Object.keys(colInfo)[0]);
