...
async function findAll({ page = 1, limit = 50, q, filters = {}, sort } = {}) {
  const offset = (page - 1) * limit;
  const qb = knex(TABLE).select('*');
  if (q) qb.where('response', 'like', '%' + q + '%');
...
async function findById(id) { return knex(TABLE).where({ id }).first(); }
async function create(data) { const [id] = await knex(TABLE).insert(data); return findById(id); }
async function update(id, data) { await knex(TABLE).where({ id }).update(data); return findById(id); }
async function remove(id) { return knex(TABLE).where({ id }).del(); }
