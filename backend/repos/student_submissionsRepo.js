...
const TABLE = 'student_submissions';
...
async function findAll({ page = 1, limit = 20, q, filters = {}, sort } = {}) {
  ...
  const pk = cols.includes('id') ? 'id' : (cols.includes('submission_id') ? 'submission_id' : cols[0]);
...
async function findById(id) {
  const colInfo = await knex(TABLE).columnInfo();
  const pk = colInfo.id ? 'id' : (colInfo.submission_id ? 'submission_id' : Object.keys(colInfo)[0]);
