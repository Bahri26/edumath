const repo = require('../repos/studentsRepo');

async function list(req, res) {
  const { page = 1, limit = 20, q, sort } = req.query;
  const filters = {};
  if (req.query.cefr_level) filters.cefr_level = req.query.cefr_level;
  try {
...
async function create(req, res) {
  try { const item = await repo.create(req.body); res.status(201).json(item); } catch (err) { res.status(500).json({ error: err.message }); } }
...
async function remove(req, res) {
  const id = Number(req.params.id);
  if (!id) return res.status(400).json({ error: 'invalid id' });
  try { await repo.remove(id); res.status(204).end(); } catch (err) { res.status(500).json({ error: err.message }); } }
